...
export class SearchComponent implements OnInit {
  ...
  ngOnInit() {
    this.users$ = this.term.valueChanges.pipe(
      ...
      // Je prends la dernier element emis
      // et pas le dernier element arrivée (le requete au server ne prenne pas le meme temps)
      // afin d'être synchro avec la frappe utilisateur
      // tap( value => console.table('2.liste finale : ', value)) // log
    );
  }
}
